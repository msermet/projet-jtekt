...
$filteredPatterns = array_filter($patternMois, function ($pattern) use ($idLigne, $annee, $mois) {
    $prod = $pattern->getProduit();
    return ((int)$pattern->getAnnee() === (int)$annee)
        && ((int)$pattern->getMois() === (int)$mois)
        && ($prod !== null && (int)$prod->getLigne() === (int)$idLigne);
});
...
<script>
    document.addEventListener('DOMContentLoaded', () => {
        ...
        tableBody.querySelectorAll('.sebango-input').forEach(input => {
            ...
            // On filtre non seulement sur le code sebango mais aussi sur la ligne (idLigne)
